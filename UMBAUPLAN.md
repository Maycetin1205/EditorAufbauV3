...
Dieser Plan ist der Arbeitsauftrag. Etappen strikt in Reihenfolge, jede endet mit einem Commit. Beim Abarbeiten Haken in diese Datei setzen — sonst wird sie nicht angefasst. **ZUERST gilt der Abschnitt „Sanierung (Nutzer-Auftrag 2026-09-01)" weiter unten — erst wenn dort alles abgehakt ist, geht es an die übrigen Etappen.**
...
- [ ] **S3 — Kommentar-Schnitt.** Bleiben darf nur: SoftEngine-Kontrakt oder Einzeiler-Warum. Fliegen: Datumsangaben, „Nutzer-Ansage …", Befund-/Plan-Querverweise (G4, C2, B1, „Regel 2" …), Erzähl-Absätze, Banner. **Ausgenommen: Kommentare INNERHALB von `css\`…\``-Vorlagen** — das sind Zeichenketten-Bytes des Bündels, sie zu löschen änderte `ff-runtime.js`. **Im Zweifel bleibt der Kommentar stehen.** Diese Etappe darf ausnahmsweise in mehreren Commits laufen (je Ordner-Paket, z. B. `src/blocks/` · `src/state/`+`src/softengine/` · Rest), jedes Paket einzeln beweisbar; der Haken kommt mit dem letzten. Beweis: `npm run build:runtime` → Bündel byte-identisch + Referenzabzug grün + check/test grün.
- [ ] **S4 — Papier.** `CLAUDE.md` neu schreiben (kurz: Nordstern, SE-Kontrakte, Nutzer-Sperrliste, wichtige Stellen — Chronik und Korrekturtabelle raus). **Die Abschnitte „SoftEngine-Kontrakte" und „Feste Zusagen" wandern WORTGLEICH mit — nie umformulieren, nie „straffen":** dort steckt Echttest-Wissen, jede Umformulierung kann einen Kontrakt verfälschen. Tote Verweise raus (z. B. `check:regeln`/`check:runtime`, `UMBAU-PLAN-V6.md`). Diese Datei hier auf die offene Arbeit eindampfen (Chronik raus, git hat sie); **offene Etappen, Arbeitsregeln und Sperre bleiben dabei wortgleich stehen.** Kein Code.
...
**Arbeitsregeln für jede Sitzung — niemand liest gegen, diese Regeln sind das einzige Netz:**

1. Vor Beginn `git fetch` + `git pull`. Danach sofort `npm run check` + `npm test`: schon VOR der ersten eigenen Änderung rot → STOPP, nichts anfassen, dem Nutzer melden.
2. Zeigt `git status` geänderte Dateien, die nicht von dir stammen (es wird zeitweise parallel gebaut): liegen lassen — nie stagen, nie committen, nie „reparieren". Im Zweifel, wessen Änderungen das sind: STOPP, Nutzer fragen.
3. Nächste offene Etappe = oberste ohne Haken, **P vor S**. Eine Sitzung = eine Etappe.
4. Bauen → `npm run check` + `npm test` grün. Wird `src/blocks/` oder von dort Importiertes berührt: `npm run build:runtime`, das Bündel gehört mit in den Commit.
5. Der Referenzabzug (`src/export/referenzabzug.test.ts`) muss byte-gleich bleiben. Ihn zu erneuern (`REFERENZ_ERNEUERN=1 npx vitest run src/export/referenzabzug.test.ts`; PowerShell: `$env:REFERENZ_ERNEUERN='1'; npx vitest run src/export/referenzabzug.test.ts`) erlauben NUR Etappen, die es ausdrücklich sagen (P5, S7) — überall sonst heißt Rot: der eigene Umbau hat den Export verändert, den Fehler bei dir suchen.
6. **Niemals einen Test abschwächen, löschen oder überspringen, damit er grün wird.** Bleibt etwas nach zwei ehrlichen Anläufen unverstanden rot: die eigenen Dateien zurücksetzen (`git restore <deine Dateien>`), Etappe offen lassen, Befund melden. Rot wird NIE committet.
7. EIN Commit je Etappe, Dateien namentlich stagen (nie `git add -A`); der frisch gesetzte Haken in dieser Datei gehört mit hinein. Danach **`git push`** (das Repo ist auf GitHub veröffentlicht und muss aktuell bleiben, Nutzer-Ansage 2026-09-01); wird der Push abgewiesen: `git pull` (Merge), erneut pushen — NIE `--force`.
8. Bericht an den Nutzer: Beweise grün/rot · Klickanleitung (was öffnen, was tun, was zu sehen sein muss) · was nicht prüfbar war. Keine „was wurde geändert"-Erzählung.
9. Neue Testfälle nur als vitest (bestehende Gattung), keine neuen Testarten; kein Dev-Server, kein Browser-Klicken, keine Screenshots — Browser- und SoftEngine-Prüfung macht allein der Nutzer.

- [x] **P1 — Speichern/Laden ehrlich (`src/state/`).** ✅ 2026-09-01 (Rot-Probe: 6 der 7 neuen Fälle fallen gegen den alten Code) (a) `notfallkopie.ts`: die Kopie bei JEDER Beschädigung sichern (Schlüssel mit Zeitzusatz statt Einmal-Wächter) und die Meldung darf nie „gesichert" behaupten, wenn nichts geschrieben wurde. (b) `VorlagenStore.ts:52`: die von `pruefe` gemeldeten `probleme` nicht wegwerfen — dem Nutzer melden (Muster `meldeVerworfeneTypen`) und VOR dem ersten gekürzten Rückschreiben den Roh-Stand als Notfallkopie sichern. (c) `persistence.ts:71`: ein Browser-Stand mit `schemaVersion > CURRENT_SCHEMA_VERSION` wird NICHT geladen und NICHT überschrieben — sichern + melden, Editor startet leer (Muster: Datei-Weg `ladeKette.ts:263`). (d) `ladeKette.ts:46`: `absichtlichEntfernt` (z. B. Kanban-Karten ohne Zielspalte, `migrationenRoh.ts:19-23`) wird gemeldet statt verschwiegen. Beweis: je Fall ein vitest-Testfall.
- [x] **P2 — GET-Antworten nicht verwechseln (`src/softengine/`).** ✅ 2026-09-01 (Rot-Probe: 3 der 4 neuen Fälle fallen gegen den alten Code, der vierte ist die Gegenprobe) (a) `relationLader.ts:50`: `antwort.fehler` durchreichen — bei Fehler bricht der Lauf ab, `meldeFehler` auch bei `still: true`, und eine unvollständige Liste wird NICHT als vollständig veröffentlicht (heute liest sich ein Timeout als „Liste zu Ende" und schneidet Positionen stumm ab). (b) `relations.ts:190-249`: Timeout 6 s → 20 s; nach einem Timeout gilt die nächste eintreffende Antwort als VERFALLEN (Verfallsmarke), damit sie nicht den nächsten Frager auflöst — es gibt keine Ruf-Antwort-Korrelation, das ist der einzige Riegel. Beweis: Fälle in `relations.test.ts` (verspätete Antwort) und `relationLader.test.ts` (Fehlerdurchreichung). Zwei Grenzen zusätzlich gebaut, weil die Marke sonst selbst still Schaden macht: sie gilt nur VERFALL_MS lang, und ein Timeout, der selbst eine Antwort verfallen ließ, setzt keine neue Marke. Der Referenzabzug musste erneuert werden — `src/softengine/` steckt im Runtime-Bündel, das die Maske byte-weise einbettet; die Maskenbytes AUSSERHALB des Bündels sind byte-gleich geblieben (mechanisch geprüft), `referenz.sevariablen.json` unverändert.
- [x] **P3 — „Lieferung" nur bei echter Lieferung (`softengine/` + `bridge`).** ✅ 2026-09-01 (Rot-Probe: 3 der 4 neuen Fälle fallen gegen den alten Code, der vierte ist die Gegenprobe) (Der vorausgesetzte holendeQuellen-Fix ist committet: `8319a63`.) (a) `relationLader.ts:22` und `:120`: `meldeNeueDaten()` behauptet eine SoftEngine-Lieferung — ersetzen durch einen Anstoß OHNE Lieferungs-Beweis (Gegenstück zu `klingeln(false)`, neue kleine Funktion in `bridge.ts`); ein Auswahlwechsel/Hol-Lauf darf `vergissGeschriebene` nie auslösen (genau das verbietet `datenAnschluss.ts:13-16`; heute verschwinden hinausgeschickte Erfassungszeilen beim bloßen Zeilenklick). (b) `bridge.ts:100` `klingeln`: jeden Zuhörer mit try/catch rufen (Muster `antwortKlingeln:131`); `seConsume:167-169`: `letzteSignatur` erst NACH fehlerfreier Verteilung setzen — sonst legt ein einziger werfender Baustein die Hydrierung dauerhaft still. Beweis: vitest-Fälle. `meldeNeueDaten` ist damit ohne Aufrufer und gelöscht — eine Lieferung meldet nur noch der Push-Weg, der sie belegen kann. Wie bei P2 musste der Referenzabzug erneuert werden (`src/softengine/` steckt im Runtime-Bündel, das die Maske byte-weise einbettet); die Maskenbytes AUSSERHALB des Bündels sind byte-gleich geblieben (mechanisch geprüft: 6 791 Bytes identisch), `referenz.sevariablen.json` unverändert.
- [x] **P4 — Erfassung zählt und rechnet richtig (`src/blocks/tabelle/`).** ✅ 2026-09-01 (Rot-Probe: 11 der 16 Fälle fallen gegen den alten Code, 5 sind Gegenproben). Zwei Dinge sind bewusst NICHT dicht: (d) hat keinen eigenen Test — Fußzeile und Knopf lesen jetzt nachweislich dieselbe Funktion (`erfassteZeilen`), ein Zähler-Test wäre ein Komponenten-Test und damit eine neue Gattung (Regel 9). Und (a) deckt das Löschen der LETZTEN Spalte nicht ab: höchste+1 vergibt deren Nummer wieder — und das ist der Normalweg, weil der Minus-Knopf der Spaltensteuerung immer die letzte nimmt (`spaltenBearbeiten.ts:51`) und Plus hinten anhängt. Ein gespeicherter Zähler am Baustein ist dafür NICHT nötig (der Satz stand hier bis 2026-09-01 und war falsch): es genügt, beim Löschen einer Spalte alles abzuräumen, was auf ihre Kennung zeigt — dann ist die Wiederverwendung der Nummer egal. Genau das tut P5(e), erweitert um die Rechnung. (a) `spalten.ts:67` (`mitKennungen`) UND der Zwilling `migrationenRoh.ts:167` (`vergebeKennungen`) — beide synchron ändern: neue Kennung = höchste vorhandene s-Zahl + 1, NIE die niedrigste freie — gelöschte Kennungen dürfen nie wiedervergeben werden (heute zeigen Rechnung und Ketten-Parameter danach stumm auf die neue Spalte). (b) `erfassungsLauf.ts:418` (`uebernimmWerte`): beim Zurückholen bleibt der Rechnungs-Zielplatz eine Lücke — Regel: ein konfigurierter Rechnungs-Platz, dessen Wert exakt dem aus den übrigen Werten gerechneten Ergebnis entspricht, wird nicht als getippt übernommen (heute friert die Abgabemenge ein: 10→20 Tiere ändern, Menge von 10 geht ins ERP). (c) `erfassungsAnschluss.ts:45` (`vormerkungen`): auch ohne `_zurueck` zählt die unten getippte, nicht-leere Zeile mit — gleiche Mechanik wie die Korrekturzeile, Kennung dafür reservieren (heute überspringt Buchen die sichtbar ausgefüllte Zeile still). (d) `TabelleBlock.ts:623`: die Fußzeile zählt über `vormerkungen()` statt `zeilen.length` — eine Zahl für Fußzeile und Knopf, wie `vormerkStand.ts:10-13` zusagt. (e) `tabelleAnsicht.ts:96` (`sichtbareIndizes`): Suche und Sortierung lesen über `wertVon` (vorgemerkter Zellwert), wie es die Summe schon tut. Beweis: Fälle in den erfassungs*/spalten/aenderungen-Tests; `npm run build:runtime`; Referenzabzug bleibt byte-gleich.
- [x] **P5 — Export-Bestellung sauber (`src/export/` + `core/data/`).** Bewusste Exportänderungen — Referenz im selben Commit erneuern. (a) `benutzteQuellen.ts:127-136`: der `kind: 'field'`-Zweig ohne `quelleProp` löst über `zerlegeBindung` auf (wie `merkeBindung` in `:88`) — heute landet ein `quelle::code`-Token roh in der FELDER-Bestellung der eigenen Quelle (bricht laut Kontrakt die ganze Loop-Liste bzw. kippt IDB still auf `'*'`). (b) Quellen-Klarnamen eindeutig: `DataSourceForm` verweigert Doppelnamen; zusätzlich macht der Export gleiche Namen maschinell eindeutig und schreibt DENSELBEN Namen in SEFILELOOP-`ALIAS` und `FF_DATA_SOURCES.name` (die Laufzeit sucht über den Namen, `data.ts:192` — erster Treffer gewinnt; heute zeigen zwei gleichnamige Quellen stumm dieselben Daten). (c) eine IDB-Quelle ohne `idbId` ist ein Ladeproblem in `pruefeDatenquellen` statt einer stillen `ID:""`-Bestellung (`sevariablen.ts:71` gegen `:23`). (d) `serializer.ts` `guardScriptContent`: in den zwei JSON-Skripten zusätzlich jedes `<` als Unicode-Escape schreiben (Backslash + u003C, in JSON-Zeichenketten gültig) — schließt den „double escaped script"-Einstieg. (e) Bedienung am Ding statt Warnung: beim LÖSCHEN einer Spalte wird alles abgeräumt, was auf ihre Kennung zeigt — Ketten-Parameter der drei Zellen-Quellen (`ZELLEN_PARAM_QUELLEN`) auf `aus` (heute exportiert `exportMask.ts:86` dafür `-1` und die Laufzeit schreibt kommentarlos einen Leerstring ins ERP) UND die Rechnungs-Plätze der Tabelle auf leer (`rechnung.ts:24`, `RechnungsPlatz.spalte`; leer = Platz unbenutzt). Damit ist auch die P4-Lücke zu: eine wiedervergebene Kennung findet keinen alten Zeiger mehr vor. Beweis: neue Fälle in benutzteQuellen-/sevariablen-Tests + Referenzabzug bewusst erneuert. ✅ 2026-09-01 (Rot-Probe: 8 der 20 neuen Fälle fallen gegen den alten Code; die übrigen sind Gegenproben oder beschreiben neue Funktionen, die es vorher nicht gab). Zwei Dinge sind bewusst NICHT dicht: das Abräumen der Rechnung hängt in `TabelleBlock.aendere` ohne eigenen Test — die Rechenarbeit selbst liegt geprüft in `rechnungNachSpalten` (`spaltenBearbeiten.ts`), aber dass der Baustein sie ruft, prüfte nur ein Komponenten-Test und damit eine neue Gattung (Regel 9); die Ketten-Seite ist dagegen durch den Store hindurch geprüft. **Vom Nutzer am 2026-09-01 von Hand nachgeprüft** (Spalte über das ✕ am Spaltenkopf gelöscht: der zugehörige Rechnungs-Platz stand danach auf „— nicht belegt —", die übrigen unverändert, und Strg+Z stellte Spalte und Zuordnung in EINEM Schritt zurück) — die Lücke ist damit nur noch eine Test-, keine Wissenslücke. Und (b) macht Doppelnamen nur im Export eindeutig — im Editor selbst bleiben Altstände mit Doppelnamen stehen, das Formular verhindert ab jetzt nur neue. Der Referenzabzug wurde erneuert (`src/blocks/` und `src/state/` stecken im Runtime-Bündel): die Maskenbytes AUSSERHALB des Bündels sind byte-gleich geblieben (mechanisch geprüft: 6 723 Bytes davor, 58 danach), `referenz.sevariablen.json` unverändert.
- [x] **P6 — Editor-Feinheiten.** ✅ 2026-09-01 (a–g gebaut; dazu zwei Laufzeit-Sperren: eine Kette stoppt mit Meldung, wenn ihre Relation in der Maske fehlt oder wenn ein `{PINDEX}`-Schritt ohne Satznummer laufen soll. **(h) bleibt offen** — die Verlustprüfung schaltet bei Altbaustein-Resten weiter global ab.) (a) `FormFeldBlock.ts:176` (`textTpl`): der Platzhalter-Span bekommt im Platzhalter-Zweig `?data-ff-bound=${wertBindbar && this.valueField !== ''}` — der Umbenennen-Riegel sitzt heute am falschen Element, Tippen bei gesetzter Bindung geht ins Leere. (b) Undo je Tastendruck im Spaltennamen-Feld: `FieldPicker.tsx:195` hängt den Aufräum-Effekt an `titelSitzung.beenden` statt an das je Render neue Sitzungs-Objekt (oder `useEingabeSitzung` gibt ein stabiles Objekt zurück). (c) `PropControl.tsx:117`: der Quellwechsel leert auch `listenBindung`-Listen mit passendem `quelleProp` (Fall `nachschlagSpalten` — behält heute Feldcodes der alten Quelle und exportiert sie). (d) `Editor.ts:261`: verwirft `schreibWert` (`null`), trotzdem `notify()` — sonst bleibt der verworfene Text sichtbar im Baustein stehen. (e) `NumberControl`: `type="number"` schluckt das Komma je nach Browser — auf Textfeld mit `inputMode="decimal"` umstellen (die vorhandene Komma-Ersetzung wird sonst nie erreicht). (f) `feldRuntime.ts:80`: die onChange-Kette startet nicht mit leerem PINDEX, wenn sie `{PINDEX}` benutzt (Sperre analog `hatSatzNummer`). (g) `bridge.ts:178`: nach einer GEWORFENEN Anmeldung weiter versuchen statt aufgeben. (h) `ladeKette.ts:216`: Altbaustein-Reste schalten die knotenweise Verlustprüfung nicht mehr global ab — betroffene IDs beim Vergleich überspringen; `migrationenRoh.ts:182` (`kennungAnPlatz`): bei unbekanntem Platz das Original stehenlassen statt `''` zu schreiben und `feld` zu löschen. Beweis: check/test grün; wo Bausteine berührt: build:runtime; Referenzabzug byte-gleich.

### Tabellen-Umbau T (Nutzer-Auftrag 2026-09-01, Abend) — läuft VOR P6 und VOR S3–S7

Analyse am Code (Tabellen-Chat, T1): Regel 2 hält die Tabelle formal ein (kein Typ-Sondercode; Export, Ketten und Store lesen die Registry) — aber fünf Konstruktionsschäden sind belegt: (1) **Die komplette Editor-Spaltenbedienung reist in den Bytes jeder exportierten Maske mit** (Beleg: „Letzte Spalte entfernen", `ff-listen-bind`, `kopf-weg` stehen in `referenz.html`; stillgelegt nur per `data-ff-editor`-Weichen — gegen Regel 1 „Editor-Hilfen leben im BlockHost"). (2) **WYSIWYG-Löcher:** tippbare Zellen (Vorgabe „änderbar = JA"), Lösch-/Zurückhol-Kreuze und Kopfklick-Sortieren gibt es nur in der Maske, der Editor zeigt nichts davon. (3) Kennungs-Vergabe doppelt (`mitKennungen` in `blocks/tabelle/spalten.ts` ↔ `vergebeKennungen` in `state/migrationenRoh.ts`, Sync-Pflicht per Kommentar). (4) Import-Kreis `blocks/formfeld` ↔ `blocks/tabelle` (Nachschlage-Fenster baut die Tabelle, die Tabelle ruft zurück). (5) Die Belegerfassungs-Rechnung (Abgabemenge = Anzahl × Dosis × Tage) liegt in `core/data/` — Anwendungsfall-Fachlichkeit in der Allgemein-Schicht. **Grundsatz-Entscheidungen des Nutzers dazu:** die Erfassung BLEIBT in der Tabelle (kein eigener Baustein, bis ein echter zweiter Fall kommt; ein „tabellenartiger" Zweitbaustein ist ausdrücklich denkbare Zukunft) · die Maske soll nichts tun oder tragen, was nicht sichtbar angelegt ist. Rituale wie bei P/S (Arbeitsregeln oben); ein go je Etappe.

- [x] **T1 — Inventur** (Analyse, kein Code). ✅ 2026-09-01 Ergebnis = dieser Abschnitt; die vier T5-Fragen hat der Nutzer am selben Abend beantwortet (s. T5).
- [x] **T2 — Kennungs-Vergabe an EINE Stelle:** ✅ 2026-09-01 (`kennungenVergeben` in `core/blocks/listenBindung.ts`; `mitKennungen` und `vergebeKennungen` rufen sie) — `mitKennungen`/`abNummer` ziehen nach `core/blocks/listenBindung.ts` (dort wohnt schon `kennungKey`); `blocks/tabelle/spalten.ts` und `state/migrationenRoh.ts` rufen dieselbe Funktion, der Zwilling `vergebeKennungen` fällt. Beweis: bestehende spalten-/migrationenRoh-Tests grün; build:runtime; Maskenbytes außerhalb des Bündels byte-gleich.
- [x] **T3 — Import-Kreis auflösen:** (✅ 2026-09-01: `blocks/shared/nachschlagen.ts`; einzig verbliebener Querimport ist der reine TYP `Spalte` in FormFeldBlock — kein Laufzeit-Kreis) das Nachschlage-Fenster (`blocks/formfeld/nachschlagen.ts`) nach `blocks/shared/` heben; danach importiert kein Baustein-Ordner mehr aus einem anderen (grep-Beweis in beide Richtungen). Verhalten identisch — Klickprobe Nutzer: Lupe am Feld UND in der Erfassungszelle öffnen dasselbe Fenster. build:runtime; Maskenbytes außerhalb des Bündels byte-gleich.
- [ ] **T4 — Editor-Bedienung raus aus den Masken-Bytes:** `spaltenBearbeiten.ts` (Spalten-Steuerung, Spalten-Zug, Umbenennen, Feld-Picker-Ruf) lädt nur noch der Editor — über das vorhandene `editorAngaben`-Muster (`registerEditorAngaben.ts` wird vom Runtime-Einstieg `export/runtime-entry.ts` nicht geladen); der Baustein ruft die Bedienung über einen dort registrierten Haken. Der Spaltenbreiten-ZUG ist ausgenommen (heute bewusst Masken-Feature, `tabelleKoerper.ts`) — ob er das bleibt, entscheidet Frage 2 in T5. Bewusste Bündel-/Referenz-Erneuerung (die Maske wird kleiner); danach SE-Echttest durch den Nutzer.
- [ ] **T5 — Nutzer-Entscheidungen umsetzen (Antworten vom 2026-09-01, Abend):** (✅ 2026-09-01: Punkt 1 — der Editor zeigt tippbare Spalten mit ✎ im Spaltenkopf; Punkt 3 — Entf-Taste merkt die fokussierte Zeile vor, der Editor zeigt das Kreuz still an; Punkt 4 — eine neue Tabelle startet mit einer Spalte. **Offen aus Punkt 3: die Optik der Kreuze** — der Musterbogen `designsprache/` liegt nicht in diesem Repo, ohne Vorlage wird nicht gestaltet.) (1) Die Vorgabe „In der Zeile änderbar = JA" **BLEIBT** (Nutzer: „soll so bleiben") — gebaut wird nur die ehrliche Gegenseite: der EDITOR zeigt sichtbar an, welche Spalten in der Maske tippbar sind (reine Anzeige, kein Verhaltenswechsel, keine Migration nötig). (2) Spaltenbreiten-Ziehen in der fertigen Maske **BLEIBT**. (3) Die Lösch-Kreuze **BLEIBEN** — aber: Optik ist „absolut räudig" (Nutzer-Urteil), sie wird nach Musterbogen/Masken-Tokens neu gemacht, und zusätzlich merkt die **Entf-Taste** die fokussierte Zeile zum Löschen vor (hängt an der bestehenden Zeilenfokus-Navigation, `zeilenAktivierung.ts`); der Editor zeigt künftig an, dass Löschen an ist. (4) Neue Tabelle startet mit **EINER** leeren Spalte statt drei (`standardSpalten`, `spalten.ts`).
- [ ] **T6 — Rechnung raus aus `core/data/`:** als Registry-Fähigkeit des Bausteins deklariert (Muster `listenBindung`); `RechnungenBereich` liest generisch über die Registry statt der festen core-Datei. Kein Verhaltenswechsel; build:runtime; Referenz-Muster wie T2/T3.
- [ ] **T7 — Spalten ausblenden in der fertigen Maske** (Nutzer-Wunsch 2026-09-01, ausdrückliche Ausnahme von der Feature-Sperre): der Maskenbediener kann Spalten ausblenden — flüchtig wie der Breiten-Zug (gilt bis zum Neuladen der Maske), Bedienung am Spaltenkopf, Bedienform nach Musterbogen. Läuft bewusst NACH T4, damit es auf der bereinigten Struktur aufsetzt und nicht zweimal gebaut wird. Vor dem Bau klärt die Etappen-Ansage: Zusammenspiel mit Suche/Summen (arbeiten weiter über ALLE Spalten oder nur sichtbare?), mit der Erfassungszeile (der Enter-Fluss darf nie in einer unsichtbaren Zelle landen) und dem Spaltenraster.

Aus T1 notiert, KEINE Etappe (erst eigene Ansage): `frischeDatenAnfordern` ist nach Aktenlage wirkungslos (steht oben unter „Offen und NICHT gebaut") · P6 enthält drei tabellen-nahe Fixes (b, c, h), die nach T laufen.
