...
- [ ] **T2 — Kennungs-Vergabe an EINE Stelle:** `mitKennungen`/`abNummer` ziehen nach `core/blocks/listenBindung.ts` (dort wohnt schon `kennungKey`); `blocks/tabelle/spalten.ts` und `state/migrationenRoh.ts` rufen dieselbe Funktion, der Zwilling `vergebeKennungen` fällt. Beweis: bestehende spalten-/migrationenRoh-Tests grün; build:runtime; Maskenbytes außerhalb des Bündels byte-gleich.
