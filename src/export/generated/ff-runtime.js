      ...
      ${e.zeigeKopf?b`<div class="kopf" role="row" style=${W(e.cols)}>
        ${e.spalten.map((n,r)=>b`<div
          ...
          >${e.imEditor&&(e.herkunft[r]??``)!==``?b`<span class="kopf-titel">${n.titel}</span><span
                ...
                >${e.herkunft[r]}</span>`:n.titel}${!e.editable&&e.sortSpalte===r?b`<span class="sort-pfeil">${e.sortAuf?` ▲`:` ▼`}</span>`:``}${e.imEditor&&e.editable&&e.spalten.length>1?Vo(n.titel,r,t.loescheSpalte):S}</div>`)}
        ...
        ${e.zeilen.map((n,r)=>{let i=n!==null&&!e.imEditor,a=n!==null&&e.zeilenStand.istGeloescht(n),o=n===null?{status:`gebucht`,titel:``}:e.zeilenStand.statusVon(n);return b`<div
            ...
            @keydown=${e=>{if(!e.target.closest(`.zell-eingabe`)){if(e.key===`ArrowDown`||e.key===`ArrowUp`){let t=e.key===`ArrowUp`;(ds(e.target,t?-1:1)||t&&ps(e.target))&&e.preventDefault();return}e.key===`Enter`&&(e.preventDefault(),t.aktiviereZeile(n,r))}}}
          >
            ${``}
