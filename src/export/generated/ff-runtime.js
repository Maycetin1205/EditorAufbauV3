      ...
      ${e.zeigeKopf?g`<div class="kopf" role="row" style=${W(e.cols)}>
        ...
        >
          ${e.spalten.map((i,a)=>{let o=U(i.art),s=a===0&&!e.imEditor?g`<button
                ...
                >&#x2715;</button>`:v;return e.imEditor?g`<div class=${o.klasse} role="cell">${o.zelle(n[a]??``,i.zuordnung??[],{})}</div>`:g`<div class=${o.klasse} role="cell">${s}<input
              class="erf-eingabe"
              type="text"
              aria-label=${i.titel}
              .value=${n[a]??``}
              @input=${e=>t.tippeErfassteZelle(r,a,e.target.value)}
            /></div>`})}
        </div>`})}
